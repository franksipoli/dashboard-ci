<?php
	class Tipoconta_model extends MY_Model {
		/* Nome da tabela no banco de dados */	
		protected static $_table = "tbxtic";
		/* Nome do campo id na tabela */
		protected static $_idfield = "nidtbxtic";
		...
		*/
		...
		public function validaInsercao(){
			...
			$tic = $this->db->where(['nativo'=>1, 'cnometic'=>$this->descricao])->get(self::$_table)->row();
			if ($tic){
				$this->error = 'Já existe um tipo de conta com o nome "'.$this->descricao.'"';
				return false;
			}
			return true;
		}
		...
		public function validaAtualizacao(){
			...
			$tic = $this->db->where(['nativo'=>1, 'cnometic'=>$this->descricao])->where(self::$_idfield.'!=',$this->id)->get(self::$_table)->row();
			if ($tic){
				$this->error = 'Já existe um tipo de conta com o nome "'.$this->descricao.'"';
				return false;
			}
