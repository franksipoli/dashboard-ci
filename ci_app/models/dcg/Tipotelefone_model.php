<?php
	class Tipotelefone_model extends MY_Model {
		
		/* Nome da tabela no banco de dados */	
		protected static $_table = "tbxttl";
		/* Nome do campo id na tabela */
		protected static $_idfield = "nidtbxttl";
		...
		*/
		...
		public function validaInsercao()
		{
			...
			$ttl = $this->db->where(['nativo'=>1, 'cdescrittl'=>$this->descricao])->get(self::$_table)->row();
			if ($ttl){
				$this->error = 'Já existe um tipo de telefone com a descrição "'.$this->descricao.'"';
				return false;
			}
			return true;
		}
		...
		*/
		...
		public function validaAtualizacao()
		{
			...
			$ttl = $this->db->where(['nativo'=>1, 'cdescrittl'=>$this->descricao])->where(self::$_idfield.'!=',$this->id)->get(self::$_table)->row();
			if ($ttl){
				$this->error = 'Já existe um tipo de telefone com a descrição "'.$this->descricao.'"';
				return false;
			}
