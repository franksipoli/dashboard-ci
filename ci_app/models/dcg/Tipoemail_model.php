<?php
	class Tipoemail_model extends MY_Model {
		...
		protected static $_table = "tbxtem";
		/* Nome do campo id na tabela */
		protected static $_idfield = "nidtbxtem";
		...
		*/
		...
		public function validaInsercao()
		{
			...
			$tem = $this->db->where(['nativo'=>1, 'cdescritem'=>$this->descricao])->get(self::$_table)->row();
			if ($tem){
				$this->error = 'Já existe um tipo de e-mail com a descrição "'.$this->descricao.'"';
				return false;
			}
			return true;
		}
		...
		*/
		...
		public function validaAtualizacao()
		{
			...
			$tem = $this->db->where(['nativo'=>1, 'cdescritem'=>$this->descricao])->where(self::$_idfield.'!=',$this->id)->get(self::$_table)->row();
			if ($tem){
				$this->error = 'Já existe um tipo de e-mail com a descrição "'.$this->descricao.'"';
				return false;
			}
