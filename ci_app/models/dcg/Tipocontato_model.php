<?php
	class Tipocontato_model extends MY_Model {
		...
		protected static $_table = "tbxtpc";
		/* Nome do campo id na tabela */
		protected static $_idfield = "nidtbxtpc";
		...
		*/
		...
		public function validaInsercao()
		{
			...
			$tpc = $this->db->where(['nativo'=>1, 'cdescritpc'=>$this->descricao])->get(self::$_table)->row();
			if ($tpc){
		...
		*/
		...
		public function validaAtualizacao()
		{
			...
			$tpc = $this->db->where(['nativo'=>1, 'cdescritpc'=>$this->descricao])->where(self::$_idfield.'!=',$this->id)->get(self::$_table)->row();
			if ($tpc){
				$this->error = 'Já existe um tipo de contato com a descrição "'.$this->descricao.'"';
				return false;
			}
