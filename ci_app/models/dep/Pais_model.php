<?php
	class Pais_model extends MY_Model {
		...
		protected static $_table = "tbxpas";
		/* Nome do campo id na tabela */
		protected static $_idfield = "nidtbxpas";
		...
		*/
		...
		public function validaInsercao(){
			...
			$pas = $this->db->where(['nativo'=>1, 'cdescripas'=>$this->descricao])->get(self::$_table)->row();
			if ($pas){
				$this->error = 'Já existe um país com a descrição "'.$this->descricao.'"';
				return false;
			}
			return true;
		}
		...
		public function validaAtualizacao(){
			...
			$pas = $this->db->where(['nativo'=>1, 'cdescripas'=>$this->descricao])->where(self::$_idfield.'!=',$this->id)->get(self::$_table)->row();
			if ($pas){
				$this->error = 'Já existe um país com a descrição "'.$this->descricao.'"';
				return false;
			}
